...
void GameServer::startRunningLoop() {
  while (true) {
    ...
    sleep(1); //should sleep until server update?
  }
}
...
bool GameServer::processMessages(Server &server,
  ...
  for (auto &message : incoming) {
    ...
    if (message.text[0] == '/') {
      ...
      if (tokens[0] == "leave") {
        if (roomManager.getRoomFromUser(user).getName() != RoomManager::GLOBAL_ROOM_NAME){
      ...
      if (tokens[0] == "info") {
       	output << "Your id is: " << user.name <<  "\n"  
               << "You are in room: "<< roomManager.getRoomFromUser(user).getName() << "\n"
      ...
      if (tokens[0] == "whisper"){
        ...
        if(tokens.size() == 2){
          userid receiver;
          //roomManager.getRoomFromUser(user).getParticipant()  //we must change from string to uintptr_t (userid)
    ...
    outMessages.push_back(DecoratedMessage{user, output.str(), isBroadcast, receiversId});
  }
