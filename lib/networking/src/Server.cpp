...
public:
  ServerImpl(Server &server, unsigned short port, std::string httpMessage)
      : server{server}, endpoint{boost::asio::ip::tcp::v4(), port}, ioContext{},
        acceptor{ioContext, endpoint}, httpMessage{std::move(httpMessage)} {
...
public:
  Channel(boost::asio::ip::tcp::socket socket, ServerImpl &serverImpl)
      : disconnected{false}, connection{reinterpret_cast<uintptr_t>(this)},
        serverImpl{serverImpl}, streamBuf{}, websocket{std::move(socket)},
        readBuffer{serverImpl.incoming} {}

  void
  start(boost::beast::http::request<boost::beast::http::string_body> &request);
...
void Channel::start(
  ...
  websocket.async_accept(request, [this, self](std::error_code errorCode) {
    if (!errorCode) {
...
void Channel::send(std::string outgoing) {
  ...
  websocket.async_write(
      boost::asio::buffer(writeBuffer.front()),
...
void Channel::readMessage() {
  ...
  websocket.async_read(
      streamBuf, [this, self](auto errorCode, std::size_t size) {
        if (!errorCode) {
...
public:
  HTTPSession(ServerImpl &serverImpl)
      : serverImpl{serverImpl}, socket{serverImpl.ioContext}, streamBuf{} {}
...
void HTTPSession::start() {
  boost::beast::http::async_read(
      socket, streamBuf, request,
      [this, session = this->shared_from_this()](std::error_code ec,
                                                 std::size_t /*bytes*/) {
        if (ec) {
          ...
          auto channel =
              std::make_shared<Channel>(std::move(socket), serverImpl);
...
void HTTPSession::handleRequest() {
  auto send = [this, session = this->shared_from_this()](auto &&response) {
    ...
    boost::beast::http::async_write(
        socket, *sharedResponse,
        [this, session, sharedResponse](std::error_code ec,
                                        std::size_t /*bytes*/) {
          if (ec) {
  ...
  auto const badRequest = [&request =
                               this->request](boost::beast::string_view why) {
  ...
  auto addResponseMetaData = [bodySize = body.size(),
                              &request = this->request](auto &response) {
...
void ServerImpl::listenForConnections() {
  ...
  acceptor.async_accept(session->getSocket(), [this, session](auto errorCode) {
    if (!errorCode) {
      session->start();
    } else {
...
Server::buildImpl(Server &server, unsigned short port,
